...
export default function Home() {
  ...
  const [applicationAppliedList, setApplicationAppliedList] = useState<Application[]>([]);
  const [applicationRemoveList, setApplicationRemoveList] = useState<Application[]>([]);
  const [applicationPendingList, setApplicationPendingList] = useState<Application[]>([]);  
  ...
  const laodsApplication = async () => {
    try {
      const applicationData = await apiService.getAllResources("/applications/demandeur");

      const filteredapplicationDataApplied = applicationData.filter(
        (application: { status: string }) => application.status === "accepted"
      );
      const filteredapplicationDataRemoved = applicationData.filter(
        (application: { status: string }) => application.status === "rejected"
      );

      const filteredapplicationDataPending = applicationData.filter(
        (application: { status: string }) => application.status === "pending"
      );

      console.log(applicationData);

      setApplicationPendingList(filteredapplicationDataPending);
  ...
  <div className="dash__content">
    ...
    <div className="dash__overview">
      <h6 className="fw-medium mb-30">Candidatures soumises</h6>
      <div className="overview__content">
        ...
        <div className="single__overview">
          ...
          <div className="content">
            <h5 className="lh-sm">{ applicationAppliedList.length}</h5>
            <p className="font-20">candidatures acceptéés</p>
          </div>
        </div>
        <div className="single__overview">
          <div className="icon">
    ...
    "Fermé"
              ...
              <div className="d-flex gap-3 flex-wrap gap-lg-4 fw-medium">
                <div className="d-flex gap-2 align-items-center">
                  <i className="fa-light fa-location-dot" /> {list.job_listing.location}
                </div>
                <div className="d-flex gap-2 align-items-center">
                  <i className="fa-light rt-briefcase" /> {list.job_listing.contract_type}
                </div>
                <div className="d-flex gap-2 align-items-center">
                  <i className="fa-light fa-clock" /> {formatDate(list.created_at)}
                </div>
              </div>
            </div>
            <div className="d-flex gap-3 gap-xl-5 flex-wrap align-items-center">
              ...
              <div className="d-flex gap-3 flex-wrap">
                <div className="job__sallery d-flex gap-2 align-items-center">
                  <i className="fa-sharp rt-price-tag" /> Salaire : {list.job_listing.min_salary} - {list.job_listing.max_salary} FCFA
                  <span>/Mois</span>
                </div>
                <button
