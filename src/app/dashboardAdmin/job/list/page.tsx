...
export default function Home() {
  ...
  const handleSeeCandidat = async (id: number) => {
    router.push(`/dashboardAdmin/job/${id}/candidat`);
  };
  ...
  const handleChangeStatut = (id:number) => {
    Swal.fire({
      title: 'Êtes-vous sûr ?',
      text: 'Vous voulez fermer ce recrutement?',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#FF5757',
      cancelButtonColor: '#d33',
  ...
  const changeJobStatut = async (id: number) => {
    try {
      await apiService.updateResource(`job_listings`,id, {statut:0});
      loadJobs();
    } catch (error) {
      console.error(error);
    }
  ...
  const handleChangeStatutYes = (id:number) => {
    Swal.fire({
      title: 'Êtes-vous sûr ?',
      text: 'Vous voulez ouvrir ce recrutement?',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#FF5757',
      cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        changeJobStatutYes(id);
      }
    });
  };
  const changeJobStatutYes = async (id: number) => {
    try {
      await apiService.updateResource(`job_listings`,id, {statut:1});
      loadJobs();
    } catch (error) {
      console.error(error);
    }
  ...
  const loadJobs = async () => {
    try {
      const jobsData = await apiService.getAllResources('job_listings');
  ...
  //     setJobsList([...jobsList, createdJob]);
  //     setNewJob({
  //       user_id:0,
  //       title: '',
  //       description: '',
  ...
  <div className="dash__content ">
    ...
    <div className="applied__job__info radius-16">
      <div className="job__filter">
        ...
        <div className="filter__job">
          <div className="nice-select filter__select">
      ...
      {jobsList.map((list) => (
        <div key={list.id} className="single__applied__job">
          <div className="single__applied__job__content">
            <div className="icon">
              <img src={user.profile_picture_path} alt="" />
            </div>
            <div className="content">
              ...
              <div className="content__info">
                <span>
                  <i className="fa-light fa-location-dot" /> {list.location}
                </span>
                <span>
                  <i className="fa-light fa-briefcase" />
                  {list.contract_type.toLocaleUpperCase()}
                </span>
              </div>
            </div>
          </div>
          <div className="middle__content ">
            <div className="job__post">
              ...
              <div className="content">
                ...
                <span>Job dedline:{list.deadline}</span>
              </div>
            </div>
            <div className="applied__status">
              <span>{list.application.length} Applied</span>
              <button 
    onClick={() => handleSeeCandidat(list.id)} 
    ...
    Voir
          ...
          <div className="action">
            ...
            {list.statut === 1 ? (
  <button 
    onClick={() => handleChangeStatut(list.id)} 
    className="action__operation pending">
    En cours
  </button>
) : (
  <button 
    onClick={() => handleChangeStatutYes(list.id)} 
    className="action__operation pending">
    Fermé
  </button>
)}
            <button onClick={() => handleDownload(list.file)} className="action__operation pending">{list.file != null ? 'File' : ''}</button>
          </div>
        </div>
            ))}
