...
export default function Home() {
  ...
  const loadJobs = async () => {
    try {
      ...
      const filteredJobs = jobsData.filter((job: { role: number; }) => job.role == 2);
  ...
  <div className="dash__content ">
    ...
    <div className="applied__job__info radius-16">
      <div className="job__filter">
        ...
        <h6 className="current ">Liste des Candidats</h6>
        </div>
      </div>
      ...
      {jobsList.map((list) => (
        <div key={list.id} className="single__applied__job">
          ...
          <div className="action">
            ...
            <button className="action__operation pending">Nb Emploi {list.applications.length}</button>
          </div>
        </div>
            ))}
