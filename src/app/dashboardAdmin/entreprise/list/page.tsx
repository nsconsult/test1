...
export default function Home() {
  ...
  const loadJobs = async () => {
    try {
      const jobsData = await apiService.getAllResources('users');
      // console.log(jobsData);
      // const userId = USERID;
      // console.log(userId);
      
      const filteredJobs = jobsData.filter((job: { role: number; }) => job.role == 3);
  ...
  <div className="dash__content ">
    ...
    <div className="applied__job__info radius-16">
      <div className="job__filter">
        ...
        <h6 className="current ">Liste des Entrepises</h6>
        </div>
      </div>
      ...
      {jobsList.map((list) => (
        <div key={list.id} className="single__applied__job">
          <div className="single__applied__job__content">
            <div className="icon">
              <img src={list.profile.profile_picture_path} alt="" />
            </div>
            <div className="content">
              ...
              <div className="content__info">
                <span>
                  <i className="fa-light fa-location-dot" /> {list.profile.country} | {list.profile.state}
                </span>
                <span>
                  <i className="fa-light fa-briefcase" />
                  {list.profile.company_categories}
                </span>
              </div>
            </div>
          </div>
          <div className="middle__content ">
            <div className="job__post">
              ...
              <div className="content">
                <span>Crée le: {formatDate(list.created_at)}</span>
                <span>Nb Job: {list.job_listings.length}</span>
              </div>
            </div>
          ...
          <div className="action">
            ...
            <button className="action__operation pending">Nb candidat {list.applications.length}</button>
          </div>
        </div>
            ))}
